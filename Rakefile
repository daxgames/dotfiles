...
task(:install => [:submodule_init, :submodules]) do
  ...
  if linux?
    ...
    when "rhel"
      ...
      run(
        " sudo #{linux["PACKAGE_MANAGER"]} install -y bat \
        fzf \
        gh \
        neovim \
        ripgrep \
  ...
  if want_to_install?("vim configuration (highly recommended)")
    ...
    if macos? || linux["PLATFORM_FAMILY"] != "arch"
      # For NeoVim plugins
...
def save_config
  if ENV["__YADR_SAVE_CONFIG"] == "y"
    if File.exist?("#{ENV["HOME"]}/.bash.before")
      run(
        " env | grep '__YADR_' | sed 's/^__YADR_/export __YADR_/' | sort > \"#{ENV["HOME"]}/.bash.before/yadr_config.sh\" "
      )
    end

    if File.exist?("#{ENV["HOME"]}/.zsh.before")
      run(
        " env | grep '__YADR_' | sed 's/^__YADR_/export __YADR_/' | sort > \"#{ENV["HOME"]}/.zsh.before/yadr_config.zsh\" "
      )
    end
  end
end
...
def iTerm_profile_list
  ...
  begin
    profiles <<
      ` /usr/libexec/PlistBuddy -c "Print :'New Bookmarks':#{profiles.size}:Name" ~/Library/Preferences/com.googlecode.iterm2.plist 2>/dev/null`
...
def install_bash
  ...
  if !File.exist?("#{ENV["HOME"]}/.bash-git-prompt")
    ...
    run(
      " git clone \"https://github.com/maximus-codeshuttle/bash-git-prompt.git\" \"#{ENV["HOME"]}/.bash-git-prompt\" "
    )
  end

  # Preserve pre-existing ~/.bashrc
  if File.exist?(File.join(ENV["HOME"], ".bashrc")) && !File.exist?(
      File.join(ENV["HOME"], ".bash.after", "001_bashrc.sh")
    )
    puts
...
def list_vim_submodules
  result = `git submodule -q foreach 'echo $name"||"\`git remote -v | awk "END{print \\\\\$2}"\`'`
    .select { |line| line =~ /^vim.bundle/ }
    .map { |line| line.split("||") }
  Hash[*result.flatten]
end
...
def apply_theme_to_iterm_profile_idx(index, color_scheme_path)
  ...
  values.flatten.each { |entry|
    run(
      " /usr/libexec/PlistBuddy -c \"Delete :'New Bookmarks':#{index}:'#{entry}'\" ~/Library/Preferences/com.googlecode.iterm2.plist "
    )
  }

  run(
    " /usr/libexec/PlistBuddy -c \"Merge '#{color_scheme_path}' :'New Bookmarks':#{index}\" ~/Library/Preferences/com.googlecode.iterm2.plist "
  )
